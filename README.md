...
5. **分界默认逗号，可配置**：顶层用 `,` 拆参（`[]`/`()`/`{}` 内不拆）；可改为 `;`、顿号等。**禁止**把 `wolframArgSeparator` 设为 `@`（`@` 专用于 Prefix 复合；误设会回退为 `,`）。
...
| `latex-helper.wolframArgSeparator` | `,` | `∴ Fun[…]` 顶层参数分界符（可改为 `;` 等）。**禁止 `@`**（留给 Prefix 复合；误设回退为 `,`） |
